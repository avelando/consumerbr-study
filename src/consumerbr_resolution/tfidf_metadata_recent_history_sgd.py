...
RECENT_HISTORY_FEATURES = tuple(
    feature
    for days in COMPANY_HISTORY_WINDOWS_DAYS
    for feature in (
        f"company_history_rate_{days}d",
        f"log_company_history_count_{days}d",
    )
)
...
def evaluate_tfidf_metadata_recent_history_sgd():
    create_project_directories()
    ...
    print()
    print(
        "TF-IDF + metadata + recent company history "
